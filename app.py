...
REGISTROS_PROSPECTOS = [
    {
        ...
        "datos": {
            ...
            "nombre": "Prospecto",
            "apellidos": "De Prueba",
            "empresa": "Turbinas y Motores Industriales S.A. de C.V.",
            "cargo": "Director de Ingeniería",
            "email": "contacto@turbinasmotores.com",
            "telefono": "+52 55 1234 5678",
            "whatsapp": "+52 1 55 1234 5678",
            "pais_region": "México",
            "origen_prospecto": "Proyecto Especial (BSV)",
            "notas_adicionales": "Este es un registro de solo lectura diseñado para guiar a los agentes en el llenado correcto de los campos. Muestra ejemplos claros en diferentes Tollgates.",
            "macro_segmento": "MIP — Maquinaria Industrial y Equipo Pesado",
            "sub_segmento": "MIP-01 — Maquinaria Industrial",
            "actividad_economica": "333610 — Fabricación de motores de combustión interna, turbinas y transmisiones",
            "geografia": "Centro",
            "relevancia": "Alta",
            "tamano_empresa": "251 y más personas",
            "banda": "A",
            "rol_contacto": "Decision Maker - CEO, CFO, Director de Planta (Plant Manager), Director de Cadena de Suministro (Supply Chain), Director de Manufactura.",
            "num_plantas": "3",
            "codigo_xz": "Consumo Directo (Manufactura - Construcción)",
            "area_interes": "Tornillería, Rodamientos, Transmisiones de potencia mecánica",
            "puntos_interes": "Requieren alta precisión en rodamientos para turbinas de gas.",
            "calidad_contacto": "Alta",
            "ventas_12m": "<$20k",
            "actividad_lammsa": "Histórico inactivo",
            "clasif_comercial": "A",
            "justif_clasif": "Cliente potencial top tier con 3 plantas productivas en el centro del país.",
            "osp_asignado": "Octavio Velázquez Torres",
            "gerente_area": "Octavio Velázquez Torres",
            "fecha_asignacion": "2026-08-01",
            "fecha_comp_1ra_inter": "2026-08-05",
            "tipo_oportunidad": "XP — Prospecto",
            "aoi_estimado": "15",
            "primera_inter_ok": "Sí",
            "fecha_1ra_inter": "2026-08-04",
            "formato_1ra_inter": "Virtual",
            "estado_plan_bsv": "En proceso"
        }
    }
]
                                ...
                                <td style="text-align: center; white-space: nowrap;">
                                    {% if r.get('is_test') %}
                                        <form method="POST" style="display:inline;">
                                            <input type="hidden" name="action_type" value="ver_lectura">
                                            <input type="hidden" name="prospecto_id" value="{{ loop.index0 }}">
                                            <button type="submit" class="sf-btn-action-icon" title="Ver prospecto (Solo Lectura)">👁️</button>
                                        </form>
                                        <button type="button" class="sf-btn-action-icon" title="No se puede borrar el prospecto de prueba" disabled style="opacity: 0.3; cursor:not-allowed;">🗑️</button>
                                    {% else %}
                                        <form method="POST" style="display:inline;">
        ...
        <div class="sf-detail-top-bar">
            ...
            <div style="display:flex; align-items:center; gap:8px;">
                {% if modo_lectura %}
                    ...
                    {% if not registro_actual.get('is_test') %}
                    <form method="POST" style="display:inline;">
...
<script>
    ...
    // --- LÓGICA DE FICHAS DE CONTACTOS ADICIONALES (POPUP Y CRM SIN TARJETAS FALSAS) ---
    let arrayContactos = [];
    ...
    function renderizarFichasContactos() {
        ...
        if (arrayContactos.length === 0) {
            html = `<div style="font-size: 12px; color: var(--sf-text-muted); font-style: italic; padding: 16px; background: #f9f9fa; border: 1px dashed var(--sf-border); border-radius: 4px; width: 100%; text-align: center;">Aún no se han registrado contactos adicionales. En esta sección se cargarán automáticamente los contactos existentes en el CRM para esta empresa, o bien, puede agregar nuevos manualmente.</div>`;
        } else {
...
def home():
    ...
    if request.method == 'POST':
        ...
        if action_type == 'ver_lectura':
            ...
            if 0 <= prospecto_id < len(REGISTROS_PROSPECTOS):
                registro_actual = REGISTROS_PROSPECTOS[prospecto_id]
                # Bloqueo de seguridad para registros inmutables
                if registro_actual.get('is_test'):
                    modo_lectura = True
                    mensaje = "El Prospecto de Prueba es inmutable y solo admite Modo Lectura."
                else:
                    modo_lectura = False
                unlocked_idx = registro_actual.get('unlocked_idx', 0)
                active_tg = registro_actual.get('active_tg', 'TG0')
            ...
            prospecto_id = int(request.form.get('prospecto_id', -1))
            if 0 <= prospecto_id < len(REGISTROS_PROSPECTOS):
                if REGISTROS_PROSPECTOS[prospecto_id].get('is_test'):
                    mensaje = "Error: El registro de prueba no puede ser eliminado por cuestiones de seguridad."
                else:
