...
<head>
    ...
    <style>
        ...
        .highlight-yellow { background-color: #fff9c4 !important; border-color: #fbc02d !important; }
        .highlight-red { background-color: #ffebee !important; border-color: #ffcdd2 !important; }
    </style>
</head>
<body>
                                            ...
                                            <div class="sf-field-group" {% if field['tipo'] == 'Texto largo' %}style="grid-column: span 2;"{% endif %}>
                                                ...
                                                {% if field['tipo'] == 'Lista (picklist)' %}
                                                    <select name="{{ field['id'] }}" 
                                                            class="sf-select {% if field['id'] in ['actividad_economica', 'rol_contacto'] and not modo_lectura %}highlight-yellow{% endif %}"
                                                ...
                                                {% elif field['tipo'] == 'Teléfono' %}
                                                    <input type="tel" id="input-{{ field['id'] }}" name="{{ field['id'] }}" value="{{ f_val }}" class="sf-input {% if field['id'] == 'whatsapp' and not modo_lectura %}highlight-yellow{% endif %}" placeholder="{% if field['id'] == 'whatsapp' %}+52 1 81 1234 5678{% else %}+52 81 0000 0000{% endif %}" data-req="{% if field['req'] %}true{% else %}false{% endif %}" {% if modo_lectura %}disabled{% endif %} oninput="actualizarHighlights()">
                                                {% elif field['tipo'] in ['Número', 'Porcentaje (%)', 'Moneda ($)'] %}
                                                    <input type="number" step="any" name="{{ field['id'] }}" value="{{ f_val }}" class="sf-input {% if field['id'] == 'aoi_estimado' and not modo_lectura %}highlight-red{% endif %}" data-req="{% if field['req'] %}true{% else %}false{% endif %}" {% if modo_lectura %}disabled{% endif %}>
                                                {% else %}
...
<script>
    ...
    // Función de formateo de moneda con separador de miles
    function formatCurrency(input) {
        let val = input.value.replace(/\D/g, ''); // Quitar lo que no sea número
        if (val !== '') {
            val = parseInt(val, 10).toLocaleString('en-US'); // Agregar comas
        }
        input.value = val;
    }
    ...
    if request.method == 'POST':
        ...
        if action_type == 'ver_lectura':
            ...
            try:
                ...
                # Mensaje de éxito local prototipo
                mensaje = f"¡Datos de {current_active_tg} guardados correctamente! Avanzando al siguiente Tollgate."
